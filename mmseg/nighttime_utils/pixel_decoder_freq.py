...
class MSDeformAttnPixelDecoderFreqAware(BaseModule):
    ...
    def __init__(self,
        ...
        for _ in range(3):
            self.freqfusions.append(
                FreqFusion(
                    lr_channels=feat_channels,
                    hr_channels=feat_channels, 
                    scale_factor=1,
                    lowpass_kernel=5,
                    highpass_kernel=3,
                    ...
                    encoder_kernel=3,
                    encoder_dilation=1,
                    compressed_channels=(feat_channels + feat_channels) // 8, 
                    semi_conv=True,
    ...
    def forward(self, feats: List[Tensor]) -> Tuple[Tensor, Tensor]:
        ...
        for idx, f in enumerate(encoder_input_list[:-1]): # low res to high
